...
class PaanaakDevice(object):
    ...
    def __exec_http__(self,payload,https):        
        ...
        if response.status_code != 500:
            ...
            if text == 'too many request':
                ...
                data = {'message': 'Too many requests has been made recently', 'status': response.status_code}
                return data

            elif text == 'requests limit':
                self.flags['requests limit'] = True
                data = {'message': 'You have reached beyond the request limit', 'status': response.status_code}
                return data
                ...
                StringResponse = {'Relays': list(map(int, text))}
            ...
            if key in self.sensors:
                if isinstance(sensor_values[key], type(self.sensors.get(key)[0])):
                    if self.sensors.get(key)[1] == 'bool':
                        payload[key] = 1 if sensor_values.get(key)  else 0
                    else:
                        payload[key] = sensor_values.get(key)
                else:
    ...
    def send_sensors_and_relays(self, sensor_values: dict,state, https=True):
        ...
        for (k,v) in payloadRelays.items():
            payload[k]=v
        return self.__exec_http__(payload,https)
